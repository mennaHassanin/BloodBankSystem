	...
	while (RegFile >>D.id >> D.name>> D.mail >> D.password>> D.age >> D.gender >> D.bloodtype >>  D.lastdonationdate.day >> D.lastdonationdate.month >> D.lastdonationdate.year)
	{
		if (id == D.id) 
		{
			...
			if (D.age > 17 && D.age < 60 && months <= 3 && !Disease())
			{
				...
				quantity = Donation[bloodtype];
				bloodType = D.bloodtype;
				int tday = t.wDay;
				int tmonth = t.wMonth;
				int tyear = t.wYear;
				string str_day = to_string(tday);
				string str_month = to_string(tmonth);
				string str_year = to_string(tyear);
				 date = str_day + '-' + str_month + '-' + str_year; 
				blood_data blood(quantity,bloodType,date);
				vecupdate.push_back(blood);
				for (auto x : vecupdate) {
					cout << x.getQuantity() << " " << x.getType() << " " << x.getDate();
				}
				return true; 
			}
			else
